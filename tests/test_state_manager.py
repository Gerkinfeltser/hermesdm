...
from state.state_manager import (
    new_state, save_state, load_state, campaign_exists,
    list_campaigns, apply_world_change
)
...
class TestStateManager:
    ...
    def test_new_state(self):
        ...
        assert state["combat"]["active"] == False
        assert state["npcs"] == {}
    ...
    def test_campaign_exists(self):
        assert campaign_exists(TEST_ID) == False
        state = new_state(TEST_ID, "Test", "fantasy")
        save_state(TEST_ID, state)
        assert campaign_exists(TEST_ID) == True
