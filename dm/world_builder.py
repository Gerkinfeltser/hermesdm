...
3. Detectá el género y usalo como setting_type:
   - VR/IA/red/digital/cibernético/cyberpunk/post-apocalíptico → "scifi"
   - magia/dragones/reinos/castillos/espadas → "fantasy"
   - zombis/apocalipsis/supervivencia → "zombie"
   - horror/oscuro/maldito/terror/psicológico → "horror"
   - piratas/corsarios/mares/navegación → "pirate"
   - vampiro/licántropo → "horror"
   - DEFAULT si none → "fantasy"
...
9. **story_arc**: Milestones según pacing_level={pacing_level}. Cada milestone debe tener una descripción concreta de >30 palabras. Incluí "unlock_flags" en cada milestone (flags que se activan al completarlo).

10. **locations**: Generá 3-8 ubicaciones conectadas con "connections" y "prerequisites".
Formato de ubicación:
  {{
    "name": "Torre Zenith",
    "description": "...",
    "connections": ["Bosque Oscuro", "Cripta"],
    "prerequisites": [],
    "locked_until_flag": null,
    "distance_from_previous": 2,
    "obstacles": ["puerta sellada", "guardias"]
  }}
Cada ubicación DEBE conectar con al menos otra. La ubicación inicial con connections vacías es válida.
Las ubicaciones avanzadas (jefe final, tesoro) deben tener prerequisites o locked_until_flag.
...
def build_world(setting: str = "fantasy") -> dict:
    ...
    # AI-generated locations (if any) — merge into world.locations
    ai_locations = tmpl.get("locations", [])
    for loc in ai_locations:
        loc_name = loc.get("name", "")
        if not loc_name or loc_name == starting_loc:
            continue
        state["world"]["locations"][loc_name] = {
            "description": loc.get("description", ""),
            "npcs": [],
            "visited": False,
            "connections": loc.get("connections", []),
            "prerequisites": loc.get("prerequisites", []),
            "locked_until_flag": loc.get("locked_until_flag"),
            "distance_from_previous": loc.get("distance_from_previous", 1),
            "obstacles": loc.get("obstacles", []),
        }
