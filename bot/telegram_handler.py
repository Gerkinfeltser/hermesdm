...
# Cargar .env al inicio (antes de otros imports que dependan de env vars)
...
from dm.reactions import ReactionEngine
from dm.spellbook import Spellbook, create_spellbook_for_class, get_class_spell_list
from dm.npc_memory import MemoryEngine
from dm.choice_tracker import ChoiceTracker
...
async def cmd_help(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        help_text = textwrap.dedent("""
            ...
            *🎭 Campaña*
            /crearcampana <descripcion> — Crear nueva campaña (alias: /setup) (ej: `/setup dark fantasy en un puerto corrupto`)
...
async def _approve_setup(update: Update, context: ContextTypes.DEFAULT_TYPE, cs: ChatState) -> None:
    ...
    try:
        ...
        st = load_state(campaign_id)
        st["setup"] = setup_data
        # BUG FIX: Promote story_arc from AI setup to state (was nested, pacing_engine couldnt find it)
        if setup_data.get("story_arc"):
            st["story_arc"] = setup_data["story_arc"]
        st["campaign"]["status"] = "active"
        st["campaign"]["name"] = setup_data.get("setting_type", "Campaña").capitalize()

        # ── Sync location and world from lore ──────────────────────────────
        lore = setup_data.get("lore", {})
        st["campaign"]["current_location"] = lore.get("starting_location", "Unknown")
        st["campaign"]["current_location_desc"] = lore.get("starting_location_desc", "")
        st["campaign"]["main_threat"] = lore.get("main_threat", "")

        # ── Sync world state ─────────────────────────────────────────────────
        st["world"] = {
            "main_threat": lore.get("main_threat", ""),
            "factions": lore.get("factions", {}),
            "description": lore.get("starting_location_desc", ""),
            "timeline": [],
            "timeline_index": 0,
        }
        st["world"]["locations"] = {
            lore.get("starting_location", "Unknown"): {
                "description": lore.get("starting_location_desc", ""),
                "npcs": [],
                "visited": True,
            }
        ...
        if setup_npcs:
            ...
            for npc_data in setup_npcs:
                ...
                campaign_npcs[npc_id] = {
                    ...
                    "status": "ALIVE",
                    "location": lore.get("starting_location", "Unknown"),
                    "disposition": "NEUTRAL",
                    "mood": "neutral",
...
async def cmd_join(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        chat_data = context.chat_data
        cs: ChatState = chat_data.get("_hermes_state", ChatState())
                # ── Level 3: Create spellbook for spellcasters ──
        from dm.spellbook import create_spellbook_for_class, KNOWN_CASTERS, PREPARED_CASTERS
        all_casters = KNOWN_CASTERS | PREPARED_CASTERS | {'wizard', 'cleric', 'druid', 'artificer', 'ranger', 'paladin'}
        if char.player_class.lower() in all_casters:
            try:
                sb = create_spellbook_for_class(char.player_class, char.level)
                char.spellbook = sb
                # Populate known_spells from spellbook for reactions system
                all_spells = list(sb.spellbook.keys()) if hasattr(sb, 'spellbook') else []
                char.known_spells = all_spells
            except Exception:
                char.spellbook = None
                char.known_spells = []
    ...
    try:
        ...
        if cs.pending_attacks:
            ...
            if resolved["hit"] and resolved["damage"] > 0:
                ...
                defender_name = attack["defender_name"]
                defender = cs.character_for(defender_name)
                if defender:
                    dmg_result = apply_damage(defender.hp, resolved["damage"])
                    note += f"\n{dmg_result['hp_lost']} dmg to {defender_name} (HP: {defender.hp.current})"
                    # ── Death Saves: when HP hits 0 ────────────────────
                    if dmg_result.get('unconscious') or dmg_result.get('dead'):
                        ds = defender.death_saves
                        if not ds:
                            from bot.character_sheet import DeathSaves as _DS
                            ds = _DS()
                            defender.death_saves = ds
                    ...
                    if all_dead and len(all_members) > 0:
                        ...
                        if cs.combat_state and cs.combat_state.active:
                            try:
                                from bot.turn_manager import end_combat as _ec
                                _ec(cs)
                                chat_data["_hermes_state"] = cs
                            except Exception:
                                pass
                    ...
                    from state.state_manager import load_state, save_state
                    st = load_state(cs.active_campaign) if cs.active_campaign else {}
                    if st:
                        dmg = resolved["damage"]
                        enemies = st.get("enemies", {})
                        if defender_name in enemies:
                            ...
                            note += f"\n\u2694\ufe0f {dmg} dmg a {defender_name}"
                        for c in st.get("combat", {}).get("initiative_order", []):
                            if c.get("name", "").lower() == defender_name.lower() and "hp" in c:
                                c["hp"]["current"] = max(0, c["hp"]["current"] - dmg)
                                break
                        save_state(cs.active_campaign, st)
...
async def cmd_talk(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        # ── LLM-powered NPC response (with memory and secrets) ──
                # ── Level 3: NPC Memory context ──
        memory_engine = MemoryEngine(state)
        memory_ctx = memory_engine.get_dialogue_context(npc_key, npc["name"], speaker)
        ...
        # Merge Level 3 memory context
        if memory_ctx:
            npc_memory_values.append(memory_ctx)
        ...
        memory_engine.record_interaction(
            npc_key, npc["name"], speaker,
            f'{speaker} said: {message}',
            disposition_delta=0, importance=1
        )
...
async def cmd_spells(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        if spellbook:
            display = spellbook.format_spellbook()
...
async def cmd_prepare(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Handle /prepare <spell> — prepare a spell for the day (prepared casters)."""
    try:
        chat_data = context.chat_data
        cs: ChatState = chat_data.get("_hermes_state", ChatState())
        player_name = update.effective_user.first_name.lower()
        ...
        args = context.args
        if not args:
            display = spellbook.format_preparation()
            await update.message.reply_text(display, parse_mode=ParseMode.MARKDOWN)
            return

        spell_name = " ".join(args).lower()

        # Find spell level from spellbook
        found_level = None
        for lvl, spells in spellbook.spellbook.items():
            if spell_name in [s.lower() for s in spells]:
                found_level = lvl
                break
...
async def cmd_remember(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Handle /remember [npc_name] — show NPC memory or choice history."""
    try:
        chat_data = context.chat_data
        cs: ChatState = chat_data.get("_hermes_state", ChatState())
        ...
        if args:
            # Show specific NPC memory
            npc_name = args[0].lower().replace(" ", "_")
            npc_key = None
            for key in state.get("npcs", {}):
                if key == npc_name or state["npcs"][key].get("name", "").lower().replace(" ", "_") == npc_name:
                    npc_key = key
                    break

            if npc_key:
                display = memory_engine.get_npc_memory_display(npc_key)
                await update.message.reply_text(display, parse_mode=ParseMode.MARKDOWN)
            else:
                await update.message.reply_text(f"NPC '{npc_name}' not found.")
        else:
            # Show choice history
            tracker = ChoiceTracker(state)
            display = tracker.get_choice_display()
            await update.message.reply_text(display, parse_mode=ParseMode.MARKDOWN)
    except Exception as e:
...
async def cmd_disengage(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Handle /disengage — take Disengage action (no opportunity attacks)."""
    try:
        chat_data = context.chat_data
        cs: ChatState = chat_data.get("_hermes_state", ChatState())
        player_name = update.effective_user.first_name.lower()
...
async def cmd_endturn(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        if timer_seconds > 0:
            ...
            # ── Level 3: Check opportunity attacks ──
            reaction_results = []
            try:
                from dm.reactions import ReactionEngine
                reaction_engine = ReactionEngine(state if 'state' in dir() else chat_data.get("_hermes_state", {}).__dict__)
            except Exception:
...
async def cmd_shortrest(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Handle /shortrest — spend 1 hit die and recover HP."""
    try:
        chat_data = context.chat_data
        cs: ChatState = chat_data.get("_hermes_state", ChatState())
        if not cs.active_campaign:
        ...
        hp_rec = result.get("hp_recovered", 0)
        hd_left = char.hp.hit_dice_remaining
        hp_now = char.hp.current
        hp_max = char.hp.max
        warlock_note = " | 🔮 Pact Magic restored!" if char.player_class == "warlock" else ""

        chat_data["_hermes_state"] = cs

        # Sync to state
        from state.state_manager import load_state, save_state, sync_chatstate_to_state
        sync_chatstate_to_state(cs.active_campaign, cs)
        if cs.active_campaign:
            st = load_state(cs.active_campaign)
            if st:
                for k, c in st.get("characters", {}).items():
                    if c.get("name") == char.name:
                        c["hp"] = char.hp.to_dict()
                        c["spell_slots"] = char.spell_slots.to_dict() if char.spell_slots else c.get("spell_slots", {})
                        break
                save_state(cs.active_campaign, st)
...
async def cmd_longrest(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Handle /longrest — full heal, restore hit dice and spell slots."""
    try:
        chat_data = context.chat_data
        cs: ChatState = chat_data.get("_hermes_state", ChatState())
        if not cs.active_campaign:
        ...
        # Restore hit dice: total = character level
        char.hp.hit_dice_remaining = char.level

        # Restore spell slots
        if char.spell_slots:
            char.spell_slots.restore_all()

        # Reset CombatState per-round flags
        if cs.combat_state:
            cs.combat_state.per_round_flags = {"action": False, "bonus": False, "reaction": False, "movement": False}
            cs.combat_state.active = False

        hd_rec = char.hp.hit_dice_remaining
            ...
            if st:
                for k, c in st.get("characters", {}).items():
                    if c.get("name") == char.name:
                        ...
                        c["spell_slots"] = char.spell_slots.to_dict() if char.spell_slots else c.get("spell_slots", {})
                        break
                st.setdefault("combat", {})["active"] = False
                save_state(cs.active_campaign, st)
...
async def cmd_conditions(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Handle /conditions — show active conditions for the caller."""
    try:
        chat_data = context.chat_data
        cs: ChatState = chat_data.get("_hermes_state", ChatState())
        ...
        for cond_name in conditions:
            ...
            if cond:
                lines.append(f"*{cond.name}*: {cond.effects.description}")
                if cond.stat_save:
                    lines.append(f"  → Save: {cond.stat_save.upper()} DC {cond.removal_save_dc} (end of turn)")
            else:
...
def build_app() -> Application:
    ...
    app.add_handler(
        CommandHandler("crearcampana", cmd_setup, filters=filters.Chat(settings.ALLOWED_GROUP_ID) if settings.ALLOWED_GROUP_ID else None)
    )
    ...
    app.add_handler(
        CommandHandler("shortrest", cmd_shortrest, filters=filters.Chat(settings.ALLOWED_GROUP_ID) if settings.ALLOWED_GROUP_ID else None)
    )
    app.add_handler(
        CommandHandler("longrest", cmd_longrest, filters=filters.Chat(settings.ALLOWED_GROUP_ID) if settings.ALLOWED_GROUP_ID else None)
    )
    app.add_handler(
        CommandHandler("conditions", cmd_conditions, filters=filters.Chat(settings.ALLOWED_GROUP_ID) if settings.ALLOWED_GROUP_ID else None)
    )
    # Level 3 commands
    app.add_handler(
        CommandHandler("spells", cmd_spells, filters=filters.Chat(settings.ALLOWED_GROUP_ID) if settings.ALLOWED_GROUP_ID else None)
    )
    app.add_handler(
        CommandHandler("prepare", cmd_prepare, filters=filters.Chat(settings.ALLOWED_GROUP_ID) if settings.ALLOWED_GROUP_ID else None)
    )
    app.add_handler(
        CommandHandler("remember", cmd_remember, filters=filters.Chat(settings.ALLOWED_GROUP_ID) if settings.ALLOWED_GROUP_ID else None)
    )
    app.add_handler(
        CommandHandler("disengage", cmd_disengage, filters=filters.Chat(settings.ALLOWED_GROUP_ID) if settings.ALLOWED_GROUP_ID else None)
    )
    app.add_handler(
...
async def _handle_player_action(update: Update, context: ContextTypes.DEFAULT_TYPE, action_text: str) -> None:
    ...
    try:
            ...
            if st:
                for key, char_data in st.get("characters", {}).items():
                    try:
                        ...
                        pass
                # Restore spellbook and known_spells from state
                spellbooks = st.get("spellbook", {})
                for sk, sb_data in spellbooks.items():
                    if sk in cs.characters:
                        try:
                            from dm.spellbook import Spellbook
                            cs.characters[sk].spellbook = Spellbook.from_dict(sb_data)
                            cs.characters[sk].known_spells = list(cs.characters[sk].spellbook.spellbook.keys())
                        except Exception:
                            pass
        ...
        # ── Update scene/act counters for encounter pacing ────────────────────
        # Only increment on SIGNIFICANT scenes (not every /j action)
        # This prevents forced encounters after just 4 casual actions
        _significant_actions = ("attack", "cast", "skill", "travel", "explore", "dialogue", "rest")
        if result.action_type in _significant_actions:
            state["scene_count"] = state.get("scene_count", 0) + 1
        # act_counter only increments for social/exploration (not combat)
        if result.action_type in ("dialogue", "explore", "generic", "skill"):
            state["act_counter"] = state.get("act_counter", 0) + 1
        ...
        # ── Scene Director: override scene type when DM brain demands it ────
        scene_decision = None  # Saved for auto-combat initialization
        if state.get("use_scene_director", True):
            try:
                from dm.scene_director import SceneDirector
                from dm.narrative_generator import SceneType
                director = SceneDirector(state=state)
                decision = director.decide(state=state, resolution=None)
                if decision and decision.scene_type:
                    scene_type = SceneType(decision.scene_type)
                    scene_decision = decision  # Save full decision
            except Exception as e:
                import logging
                logging.getLogger(__name__).warning(f"SceneDirector failed: {e}", exc_info=True)
                pass  # Fall back to pacing engine if Director fails

        milestone_ctx = pacing.get_milestone_context()

        # Merge event injection context if available (inject_event, event_types, main_threat, etc.)
        event_ctx = pacing.get_event_context()
        if event_ctx.get("inject_event"):
            milestone_ctx.update(event_ctx)

        # ── Apply condition effects to player ─────────────────────────────────
        from dm.conditions import has_disadvantage_on_attacks, is_incapacitated, get_condition_summary

        # Check if incapacitated (can't take actions)
        if is_incapacitated(char):
            await update.message.reply_text(
                f"⛔ *{char.name}* está incapacitado y no puede actuar.\n"
                f"Condiciones: {get_condition_summary(char)}"
            )
            return
        ...
        try:
            ...
            f"Error en el motor de nuestra Master Dougeon: {e}"
            return

        if not result:
            await update.message.reply_text("No se pudo procesar la acción. Probá de nuevo.")
            return

        # --- Apply damage and check KO for attack actions (/j ataco) ---
        if result.action_type in ("attack", "cast") and result.damage and result.damage > 0 and result.target:
            target_name = result.target
            enemy_key = target_name.lower().replace(" ", "_")
            enemies = state.get("enemies", {})
            target_enemy = None
            for k, v in enemies.items():
                if k == enemy_key or v.get("name", "").lower() == target_name.lower():
                    target_enemy = v
                    enemy_key = k
                    break
            if target_enemy:
                old_hp = target_enemy.get("hp_current", 0)
                new_hp = max(0, old_hp - result.damage)
                target_enemy["hp_current"] = new_hp
                log.info("Damage applied: " + str(result.damage) + " to " + target_name + " (HP: " + str(old_hp) + " -> " + str(new_hp) + ")")
                if new_hp <= 0:
                    target_enemy["hp_current"] = 0
                    target_enemy["ko"] = True
                    result.kill = True
                    if cs.combat_state and cs.combat_state.active:
                        from bot.turn_manager import remove_combatant
                        rpc = remove_combatant(cs.combat_state, target_name)
                        if "error" not in rpc:
                            log.info(target_name + " removed from combat")
                        chat_data["_hermes_state"] = cs
                    combat_s = state.get("combat", {})
                    if combat_s.get("active"):
                        old_order = combat_s.get("initiative_order", [])
                        combat_s["initiative_order"] = [c for c in old_order if c.get("name", "").lower() != target_name.lower()]
                        remaining_enemies = [c for c in combat_s["initiative_order"] if not c.get("is_player", False)]
                        if not remaining_enemies:
                            combat_s["active"] = False
                            if cs.combat_state:
                                from bot.turn_manager import end_combat
                                cs.combat_state = end_combat(cs.combat_state)
                                chat_data["_hermes_state"] = cs
                            result.narrative += "\n\n🏆 *¡VICTORIA!* 🏆\nTodos los enemigos han sido derrotados. El combate ha terminado."
                            # ── XP Award: calculate and distribute XP from defeated enemies ──
                            try:
                                from dm.xp_engine import award_combat_xp, CR_XP
                                defeated_enemy_data = []
                                for ek, ev in state.get("enemies", {}).items():
                                    if ev.get("hp_current", 0) <= 0:
                                        defeated_enemy_data.append(ev)
                                party_char_dicts = []
                                for pname in cs.active_character_names():
                                    char_obj = cs.character_for(pname)
                                    if char_obj:
                                        party_char_dicts.append({
                                            "name": char_obj.name,
                                            "xp": char_obj.xp,
                                            "level": char_obj.level,
                                            "player_class": char_obj.player_class,
                                        })
                                if defeated_enemy_data and party_char_dicts:
                                    xp_result = award_combat_xp(party_char_dicts, defeated_enemy_data)
                                    total_xp = xp_result["total_xp"]
                                    xp_each = xp_result["xp_per_player"]
                                    result.narrative += f"\n\n✨ *+{xp_each} XP* para cada miembro del grupo (total: {total_xp})"
                                    # Apply XP to Character objects
                                    for r in xp_result.get("results", []):
                                        char_obj = cs.character_for(r["name"])
                                        if char_obj:
                                            char_obj.xp = r["xp"]
                                            if r.get("levels_gained"):
                                                for lvl_gain in r["levels_gained"]:
                                                    new_lvl = lvl_gain.get("new_level", char_obj.level + 1)
                                                    char_obj.level = new_lvl
                                                    result.narrative += f"\n\n⬆️ *¡{char_obj.name} sube a nivel {new_lvl}!*"
                                                    if lvl_gain.get("hp_gained"):
                                                        result.narrative += f" (+{lvl_gain['hp_gained']} HP máximo)"
                                    chat_data["_hermes_state"] = cs
                                    # Sync XP to state
                                    for pname in cs.active_character_names():
                                        c = state.get("characters", {}).get(pname, {})
                                        char_obj = cs.character_for(pname)
                                        if char_obj and c:
                                            c["xp"] = char_obj.xp
                                            c["level"] = char_obj.level
                                            c["hp"] = char_obj.hp.to_dict()
                                    # ── Loot: roll drops from defeated enemies ──
                                    try:
                                        from dm.loot_tables import roll_individual_treasure
                                        import random as _rand_loot
                                        total_coins = {"cp": 0, "sp": 0, "ep": 0, "gp": 0, "pp": 0}
                                        all_gems = []
                                        all_magic = []
                                        for edata in defeated_enemy_data:
                                            ecr = edata.get("cr", 0)
                                            loot = roll_individual_treasure(ecr)
                                            for denom in total_coins:
                                                total_coins[denom] += loot.get("coins", {}).get(denom, 0)
                                            all_gems.extend(loot.get("gems", []))
                                            all_magic.extend(loot.get("magic_items", []))
                                        loot_text = []
                                        gp = total_coins.get("gp", 0)
                                        if gp > 0:
                                            loot_text.append(f"💰 {gp}gp")
                                        if all_gems:
                                            loot_text.append(f"💎 {len(all_gems)} gemas")
                                        if all_magic:
                                            for mi in all_magic[:3]:
                                                loot_text.append(f"🔮 {mi.get('name', 'item mágico')}")
                                        if loot_text:
                                            result.narrative += "\n\n*Loot:* " + " | ".join(loot_text)
                                            # Add gold to party members
                                            for pname in cs.active_character_names():
                                                char_obj = cs.character_for(pname)
                                                if char_obj:
                                                    char_obj.inventory.append(type('Item', (), {'name': f'{gp}gp (loot)', 'weight': 0, 'quantity': 1})())
                                        # Save gold to state
                                        state.setdefault("party_gold", 0)
                                        state["party_gold"] = state.get("party_gold", 0) + gp
                                    except Exception as e:
                                        log.warning(f"Loot roll failed: {e}")
                            except Exception as e:
                                log.warning(f"XP award failed: {e}")
                            log.info("Combat victory in " + cs.active_campaign)
                        else:
                            result.narrative += "\n\n💀 *" + target_name + " ha caído!* Quedan " + str(len(remaining_enemies)) + " enemigo(s) en pie."
                    state["enemies"][enemy_key] = target_enemy
                    state["combat"] = combat_s

        # ── Auto-initialize combat from SceneDirector encounter ──────────────
        if scene_decision and scene_decision.requires_combat_init:
            try:
                enemies = scene_decision.enemies or []
                if enemies:
                    # Build participants list for combat system
                    participants = []
                    ...
                    for enemy in enemies:
                        ...
                        participants.append({
                            "name": enemy_name,
                            "is_player": False,
                            "dex_mod": enemy.get("dex", 10) // 2 - 5,  # Rough dex mod
                        })
                    # Start combat
                    from bot.turn_manager import start_combat as init_combat
                    cs.combat_state = init_combat(participants)
                    # Update game state for persistence
                    state["combat"] = {
                        "active": True,
                        "round": cs.combat_state.round,
                        "initiative_order": [
                            {"name": c.name, "is_player": c.is_player, "initiative": c.initiative}
                            for c in cs.combat_state.initiative_order
                        ],
                        "current_turn": cs.combat_state.current_turn,
                    }
                    # Add enemy full stats to state for tracking
                    for enemy in enemies:
                        enemy_name = enemy.get("name", "Unknown Enemy")
                        state.setdefault("enemies", {})[enemy_name] = {
                            "hp_current": enemy.get("hp", 30),
                            "hp_max": enemy.get("hp", 30),
                            "ac": enemy.get("ac", 12),
                            "cr": enemy.get("cr", 1),
                            "attacks": enemy.get("attacks", [{"name": "Attack", "to_hit": 2, "damage": "1d6"}]),
                            "multiattack": enemy.get("multiattack", 1),
                            "features": enemy.get("features", []),
                            "creature_type": enemy.get("creature_type", "unknown"),
                        }
                    # Add combat start to narrative
                    initiative_order = ", ".join(
                        f"{c.name} ({c.initiative})"
                        for c in cs.combat_state.initiative_order[:4]
                    )
                    result.narrative += (
                        f"\n\n⚔️ *¡COMBATE INICIADO!*\n"
                        f"Iniciativa: {initiative_order}\n"
                        f"Turno de *{cs.combat_state.current_turn}*."
                    )
                    log.info(
                        f"Auto-combat initialized: {len(enemies)} enemies, "
                        f"turn: {cs.combat_state.current_turn}"
                    )
            except Exception as e:
                log.exception(f"Auto-combat init failed: {e}")

        # ── Enemy turns: after player action, enemies attack back ──────────
        if (state.get("combat", {}).get("active") and
            state.get("enemies") and
            result.action_type in ("attack", "cast", "skill", "dialogue", "explore", "generic", "rest")):
            try:
                from dm.enemy_turn import EnemyTurnResolver
                resolver = EnemyTurnResolver(state, cs.characters)
                enemy_result = resolver.resolve_enemies()

                if enemy_result.narrative:
                    result.narrative += f"\n\n{enemy_result.narrative}"

                # Handle death saves for players hit by enemies
                for player_name in enemy_result.players_unconscious:
                    char_obj = cs.character_for(player_name)
                    if char_obj and char_obj.hp.current <= 0:
                        ds = char_obj.death_saves
                        if not ds:
                            from bot.character_sheet import DeathSaves as _DS
                            ds = _DS()
                            char_obj.death_saves = ds
                        stabilized, dead = ds.roll()
                        ds_status = f"successes={ds.successes}/3 failures={ds.failures}/3"
                        if dead:
                            result.narrative += f"\n\n💀 ¡{player_name} ha MUERTO! {ds_status}"
                        elif stabilized:
                            result.narrative += f"\n\n✨ ¡{player_name} se ha ESTABILIZADO! {ds_status}"
                            char_obj.hp.current = 1
                        else:
                            result.narrative += f"\n\n🫀 Death save ({player_name}): {ds_status}"

                for player_name in enemy_result.players_dead:
                    char_obj = cs.character_for(player_name)
                    if char_obj:
                        char_obj.hp.current = 0
                    result.narrative += f"\n\n☠️ ¡{player_name} ha MUERTO por daño masivo!"

                # ── Condition removal: check end-of-turn saves ────────────────
                from dm.conditions import get_condition, roll_saving_throw
                for pname in list(cs.characters.keys()):
                    pchar = cs.characters[pname]
                    if not hasattr(pchar, 'conditions') or not pchar.conditions:
                        continue
                    for cond_name in list(pchar.conditions):
                        cond = get_condition(cond_name)
                        if cond and cond.save_dc_type == "end_of_turn" and cond.stat_save:
                            save_result = roll_saving_throw(
                                pchar, cond.stat_save, cond.removal_save_dc
                            )
                            if save_result["success"]:
                                pchar.remove_condition(cond_name)
                                result.narrative += (
                                    f"\n✨ *{pname}* se libra de *{cond.name}*! "
                                    f"({save_result['breakdown']})"
                                )
                            else:
                                result.narrative += (
                                    f"\n🫀 *{pname}* falla save vs *{cond.name}*. "
                                    f"({save_result['breakdown']})"
                                )

                # Sync HP + conditions to state after enemy attacks
                from state.state_manager import load_state, save_state
                if cs.active_campaign:
                    st = load_state(cs.active_campaign)
                    if st:
                        for k, c in st.get("characters", {}).items():
                            if c.get("name") in cs.characters:
                                char_in_cs = cs.characters[c["name"]]
                                if hasattr(char_in_cs, 'hp'):
                                    c["hp"] = char_in_cs.hp.to_dict()
                                c["conditions"] = getattr(char_in_cs, 'conditions', [])
                        save_state(cs.active_campaign, st)

            except Exception as e:
                log.warning(f"Enemy turn resolution failed: {e}", exc_info=True)

        # ── Record scene FIRST, THEN check milestone advance ───────────────────
        # BUG FIX: scene_count must be current when checking advance conditions
        ...
        # ── Parse location from narrative and update state ─────────────────────
        # CRITICAL: If the narrative mentions arriving at a new location, update state
        try:
            import re
            narrative_lower = result.narrative.lower()
            location_patterns = [
                r"lleg[áa]s? a ([^.,]+)",
                r"entra[áa]s? a(?:l?|la|los|las)? ([^.,]+)",
                r"te dirig[ií]s? a(?:l?|la|los|las)? ([^.,]+)",
                r"acerc[áa]s? a(?:l?|la|los|las)? ([^.,]+)",
                r"vas a(?:l?|la|los|las)? ([^.,]+)",
                r"voy a(?:l?|la|los|las)? ([^.,]+)",
                r"te encontr[áa]s? en ([^.,]+)",
                r"est[áa]s? en ([^.,]+)",
                r"en ([^.,]+(?:ciudad|aldea|templo|santuario|dungeon|torre|cueva|palacio|mazmorra|fortaleza|refugio|base|guarida|nido|塔|oasis))",
            ]
            current_loc = (state.get("campaign", {}).get("current_location") or "").lower()
            for pattern in location_patterns:
                match = re.search(pattern, narrative_lower)
                if match:
                    new_loc_raw = match.group(1).strip()
                    # Clean up common artifacts
                    new_loc_clean = re.sub(r'^(el|la|los|las|un|una|unos|unas)\s+', '', new_loc_raw)
                    new_loc_clean = new_loc_clean.strip().title()
                    # Skip if same as current
                    if new_loc_clean.lower() != current_loc and len(new_loc_clean) > 2:
                        state.setdefault("campaign", {})["current_location"] = new_loc_clean
                        # Add to world.locations if new
                        state.setdefault("world", {}).setdefault("locations", {})[new_loc_clean] = {
                            "description": f"Nueva ubicación descubierta: {new_loc_clean}",
                            "npcs": [],
                            "visited": True,
                        }
                        log.info(f"Location changed to: {new_loc_clean}")
                        break
        except Exception:
            log.exception("Location parse from narrative failed")

        # Save updated story_arc back to state
        state["story_arc"] = pacing.arc.to_dict()

        # Write to history so LLM knows what happened in previous scenes
        history = state.get("history", [])
        history.append({
        ...
        # Level 3: Record player choice in choice tracker
        try:
            from dm.choice_tracker import ChoiceTracker
            tracker = ChoiceTracker(state)
            # Track combat vs non-combat decisions
            if result.action_type in ("attack", "cast"):
                tracker.record_choice(
                    description=f"Combat action: {action_text[:80]}",
                    category="combat",
                    options=["fight", "flee", "negotiate"],
                    chosen="fight",
                )
            elif result.action_type == "dialogue":
                tracker.record_choice(
                    description=f"Social action: {action_text[:80]}",
                    category="social",
                    options=["cooperate", "deceive", "intimidate", "ignore"],
                    chosen="interact",
                )
            elif result.action_type == "explore":
                tracker.record_choice(
                    description=f"Exploration: {action_text[:80]}",
                    category="exploration",
                    options=["investigate", "advance", "rest"],
                    chosen="explore",
                )
            tracker.save()
        except Exception:
            pass  # Don't break game if choice tracking fails
